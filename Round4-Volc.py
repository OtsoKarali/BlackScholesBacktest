...
# =============================
PARAMS = {
    Product.VOLCANIC_ROCK_VOUCHER_9500: {"mean_volatility": 0.154105, "strike": 9500, "starting_time_to_expiry": 4/252}, #0.1463
    Product.VOLCANIC_ROCK_VOUCHER_9750: {"mean_volatility": 0.161, "strike": 9750, "starting_time_to_expiry": 4/252}, #0.3106
    Product.VOLCANIC_ROCK_VOUCHER_10000: {"mean_volatility": 0.147766, "strike": 10000, "starting_time_to_expiry": 4/252}, #0.2721
    Product.VOLCANIC_ROCK_VOUCHER_10250: {"mean_volatility": 0.138391, "strike": 10250, "starting_time_to_expiry": 4/252}, #0.2665
    Product.VOLCANIC_ROCK_VOUCHER_10500: {"mean_volatility": 0.141255, "strike": 10500, "starting_time_to_expiry": 4/252}, #0.2933
    # Pairs Trading
...
def cal_tau(day, timestep, T=4/252):
    return T - ((day - 1) * 20000 + timestep) * 2e-7

def cal_call(S, tau, sigma, r=0, K=10000):
...
def cal_imvol(market_price, S, tau, r=0, K=10000, tol=1e-6, max_iter=100):
    if market_price <= 0 or S <= 0 or tau <= 0:
        return None
    sigma = 0.16  # Initial guess
    diff = cal_call(S, tau, sigma, K=K)[0] - market_price
    iter_count = 0
    while abs(diff) > tol and iter_count < max_iter:
        vega = (cal_call(S, tau, sigma + tol, K=K)[0] - cal_call(S, tau, sigma, K=K)[0]) / tol
        if vega == 0:
            break
        sigma -= diff / vega
        diff = cal_call(S, tau, sigma, K=K)[0] - market_price
        iter_count += 1
    return sigma if iter_count < max_iter else None
...
class MacaronsStrategy(Strategy):
    ...
    def macarons_adap_edge(self, timestamp: int, curr_edge: float, traded_volume: int, traderObject: dict) -> float:
        if timestamp == 0:
    ...
    def macarons_clear_position(self, order_depth: OrderDepth, position: int, orders: List[Order], buy_order_volume: int, sell_order_volume: int) -> tuple[List[Order], int, int]:
        ...
        if position > self.params[self.symbol]["position_clear_threshold"]:
            clear_quantity = sum(volume for price, volume in order_depth.buy_orders.items() if price >= best_bid - 2)
            ...
            clear_quantity = sum(abs(volume) for price, volume in order_depth.sell_orders.items() if price <= best_ask + 2)
    ...
    def act(self, state: TradingState) -> None:
        ...
        adap_edge = self.macarons_adap_edge(state.timestamp, traderObject[self.symbol]["curr_edge"], traded_volume, traderObject)
        take_orders, buy_order_volume, sell_order_volume = self.macarons_arb_take(state.order_depths[self.symbol], state.observations.conversionObservations[self.symbol], adap_edge, position)
        clear_orders, buy_order_volume, sell_order_volume = self.macarons_clear_position(state.order_depths[self.symbol], position, take_orders, buy_order_volume, sell_order_volume)
        make_orders, _, _ = self.macarons_arb_make(state.order_depths[self.symbol], state.observations.conversionObservations[self.symbol], position, adap_edge, buy_order_volume, sell_order_volume)
    ...
    def run(self, state: TradingState, trader_data: Dict) -> Dict[str, List[Order]]:
        ...
        for voucher in [
            ...
            if not voucher_order_depth.buy_orders or not voucher_order_depth.sell_orders:
                continue
            market_mid_price = self.get_mid_price(voucher_order_depth)
            params = PARAMS[voucher]
            sigma = params["mean_volatility"]
            K = params["strike"]
            tau = params["starting_time_to_expiry"] - (state.timestamp / 1_000_000 / 250)
            if tau <= 0:
                tau = 1 / (250 * 100)
            theo, delta = cal_call(rock_mid_price, tau, sigma, K=K)
            iv = cal_imvol(market_mid_price, rock_mid_price, tau, K=K)
            position = self.get_position(state, voucher)
            ...
            if iv is not None:
                ...
                if vol_spread > threshold:
                    sell_amount = min(self.limits[voucher] + position, voucher_order_depth.buy_orders.get(max(voucher_order_depth.buy_orders.keys()), 0))
                    if sell_amount > 0:
                        orders.append(Order(voucher, max(voucher_order_depth.buy_orders.keys()), -sell_amount))
                elif vol_spread < -threshold:
                    buy_amount = min(self.limits[voucher] - position, -voucher_order_depth.sell_orders.get(min(voucher_order_depth.sell_orders.keys()), 0))
                    if buy_amount > 0:
                        orders.append(Order(voucher, min(voucher_order_depth.sell_orders.keys()), buy_amount))
            if orders:
                result[voucher] = orders
        rock_position = self.get_position(state, Product.VOLCANIC_ROCK)
        target_rock_position = -round(total_delta)
        hedge_quantity = target_rock_position - rock_position
        rock_orders = []
        if hedge_quantity > 0 and rock_order_depth.sell_orders:
            best_ask = min(rock_order_depth.sell_orders.keys())
            quantity = min(hedge_quantity, -rock_order_depth.sell_orders[best_ask])
            quantity = min(quantity, self.limits[Product.VOLCANIC_ROCK] - rock_position)
            if quantity > 0:
                rock_orders.append(Order(Product.VOLCANIC_ROCK, best_ask, quantity))
        elif hedge_quantity < 0 and rock_order_depth.buy_orders:
            best_bid = max(rock_order_depth.buy_orders.keys())
            quantity = min(-hedge_quantity, rock_order_depth.buy_orders[best_bid])
            quantity = min(quantity, self.limits[Product.VOLCANIC_ROCK] + rock_position)
            if quantity > 0:
                rock_orders.append(Order(Product.VOLCANIC_ROCK, best_bid, -quantity))
        if rock_orders:
...
class Trader:
    def __init__(self):
        self.limits = {
            ...
            Product.CROISSANTS: 0, #250
            Product.JAMS: 0, #350
            Product.DJEMBES: 0, #60
            Product.PICNIC_BASKET1: 0, #60
            Product.PICNIC_BASKET2: 0, #100
